...
import os
from join_cup_hand import YellowContourDetectorWithMediaPipe
class RobotMain(object):
    ...
    def __init__(self, robot,topping,**kwargs):
        ...
        self.order_msg='A'
        self.speaker_thred=None
        self.speaker_thred_running=queue.Queue()
        ...
        self.camera=YellowContourDetectorWithMediaPipe()
    ...
    def check_camera_status(self):
        try:
            while True:
                if self.camera.text == "False":
                    # self.alive = False
                    self.arm.set_state(3)
                    #self.pprint("Paused due to camera status: False")
                elif self.camera.text == "True": #and not self.alive:
                    #self.alive = True
                    ...
                    self._arm.motion_enable(True)
                    self._arm.set_mode(0)
                    self._arm.set_state(0)  # 스포츠 모드로 복귀
                    #time.sleep(0.5)
                    
                    # 상태 복구
                    self.alive = True
                    self.is_safety_stop = False
                    #self.pprint("Resuming operation due to camera status: True")
                time.sleep(0.5)  # Check every second
        except Exception as e:
            self.pprint(f"Error in check_camera_status: {e}")
        finally:
            self.pprint("Camera status checker stopped.")

#----------------------------------------vision------------------------------------------------------------------

    def load_and_predict( self,camera_id=2, size=(224, 224)):
        # TensorFlow version check
        print('vision', tf.__version__)
        class_names=['A','B','C','D']
        ...
        while cap.isOpened():
            ...
            # Image processing
            h, w, _ = img.shape
            img = img[:, 200:200+img.shape[0]]  # Crop the image (optional)
            ...
            self.text.put(class_names[idx])
            # if class_names[idx]=='D':
            #     self.speaker_thred_running.put(True)
            # else:
            #     self.speaker_thred_running.put(False)
            # Draw the predicted label on the image
            cv2.putText(img, text=class_names[idx], org=(10, 30), fontFace=cv2.FONT_HERSHEY_SIMPLEX, fontScale=0.8, color=(0, 0, 255), thickness=2)
...
#----------------------------------------motion------------------------------------------------------------
    ...
    def motion_grab_capsule(self):
        ...
        while True:
            image=self.camera.img
            image=image[10:90,440:510]
            ...
            # 예측 수행
            prediction = self.camera.model.predict(img_input, verbose=0)
        
            idx = np.argmax(prediction)
            state=self.class_names[idx]
            if state=='Yes':
                break
            else:
                print("speak!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
                self.speak("아이스크림 캡슐을 놓아 주세요")
                time.sleep(2)
    ...
    def motion_serve(self):
        ...
        self._tcp_speed = 100
        self._tcp_acc = 1000
        # self.speaker_thred=threading.Thread(target=self.speak())
        # self.speaker_thred.start()
        # while True:
        #     if self.camera.info!='D':
        #     #if self.text.get()!="D":
        #         #print("@@@@@@@@@@@@@@@", self.text.get())
        #         break
        #     elif self.camera.info=='D':
        #     #elif self.text.get()=='D':
        #         if self.speaker_thred is None or not self.speaker_thred.is_alive():
        #             self.speaker_thred=threading.Thread(target=self.speak,args=("아이스크림을가져가세요",))
        #             self.speaker_thred.start()
        #             print("speak!!!!!!!!!!!!!!!!")
        ...
        while True:
            ...
            if state=='None':
                ...
                print("speak!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
                self.speak("아이스크림을 가져가세요")
                time.sleep(2)
            
        # self.speaker_thred.join()
        if self.order_msg== 'A':
    ...
    def run(self):
        try:
            # vision_thread = threading.Thread(target=self.load_and_predict)
            # vision_thread.daemon = True  # 프로그램 종료 시 스레드 종료
            # vision_thread.start()
             # Start the camera status checking thread
            camera_check_thread = threading.Thread(target=self.check_camera_status)
            camera_check_thread.daemon = True
            camera_check_thread.start()

            thread = threading.Thread(target=self.camera.process_frame)
            thread.daemon=True
            thread.start()
